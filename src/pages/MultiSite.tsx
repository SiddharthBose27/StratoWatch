...
export default function MultiSite() {
  ...
  // Add selected files (append until we reach siteCount)
  ...
  function replaceAll(list: FileList | null) {
    if (!list) return;
    const incoming = Array.from(list).slice(0, siteCount);
    setFiles(incoming);
  }
  ...
  async function run() {
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        ...
        throw new Error(
          `Backend did not return JSON (HTTP ${res.status}). ${text.slice(0, 200)}`,
        );
      }

      const data = await res.json();
      if (!res.ok || !data.ok) {
        const msg = data?.error || data?.detail || `Request failed (HTTP ${res.status})`;
  ...
  return (
    <div className="px-6 md:px-12 py-14">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-4 grid gap-10 lg:grid-cols-2 lg:items-start">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-5 glow-hover">
                ...
                <div className="mt-2 text-white/55 text-sm">
                  Multiple CSV/XLSX files (one per station) + site count.
                </div>
              </div>
              <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-5 glow-hover">
          ...
          <div className="rounded-3xl bg-white/5 ring-1 ring-white/10 p-8 glow-hover relative overflow-hidden">
            ...
            <div className="relative grid gap-6">
              {/* SITE COUNT */}
              <div>
                <div className="text-white/80 font-medium">How many sites?</div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSiteCount(n);
                    setFiles((prev) => prev.slice(0, Math.max(0, n)));
                  }}
                ...
                {siteCount === 7 ? (
                  <div className="mt-3 text-sm text-emerald-400">
                    ✅ Recommended — trained checkpoints available for 7 sites.
                  </div>
                ) : (
                  <div className="mt-3 text-sm text-amber-300">
                    ⚠ Best performance is with 7 sites. Other counts run in
                ...
                <div className="mt-2 text-xs text-white/45">
                  Required files:{" "}
                  <span className="text-white/70">{siteCount}</span> | Selected:{" "}
                ...
                <div className="mt-3 rounded-2xl bg-white/5 ring-1 ring-white/10 p-6 glow-hover">
                  ...
                  <label className="block cursor-pointer">
                    ...
                    <div className="text-white/60 text-sm">
                      {files.length === 0 ? (
                        "Click to add datasets (CSV/XLSX)"
                      ) : remaining > 0 ? (
                        <>
                          <div className="text-white/80 font-medium">
              ...
              {/* MODEL SELECT */}
              <div>
                <div className="text-white/80 font-medium">Model variant</div>
        ...
        {/* WARNING BANNER */}
        {warning && (
          <div className="mt-6 rounded-2xl bg-yellow-400/10 ring-1 ring-yellow-400/30 p-5 text-yellow-300 text-sm whitespace-pre-line">
            {warning}
          </div>
        )}
            ...
            <>
              ...
              {metrics && (
                <div className="mt-8 grid gap-6 md:grid-cols-3">
                  {"MAE" in metrics && (
                    <MetricCard label="MAE" value={fmt(metrics.MAE)} />
                  )}
                  {"RMSE" in metrics && (
                    <MetricCard label="RMSE" value={fmt(metrics.RMSE)} />
                  )}
                  {"R2" in metrics && (
                    <MetricCard label="R²" value={fmt(metrics.R2)} />
                  )}
                </div>
              )}
